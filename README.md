...
유저명이나 스테이지 정보와 같이, 게임 전체에 걸쳐 사용되는 데이터를 다루기 위해서는 특정 Level의 데이터를 다른 Level까지 유지할 필요가 있었습니다.

그러나 언리얼 엔진에서는 Level을 이동할 때 대부분의 오브젝트가 새롭게 생성되기에, 데이터 유지에 어려움이 있었습니다. 이를 해결하기 위해, 게임 전반에 걸쳐 유지되는 GameInstance를 파악하고 활용하여, 데이터의 Level 이동을 구현했습니다.

위 방법을 통해, Lobby의 스테이지 정보를 선택한 Stage레벨에 갖고와 적용시킬 수 있으며, 이후 Stage의 최종 점수를 Lobby로 가져가 스테이지 액터를 업데이트 할 수 있습니다.
